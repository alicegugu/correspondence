...
static double _frustum_size = 60.0;
 ...
 void zprReshape(int w,int h)
{
    ...
    _top    =  1.0*_frustum_size;
    _bottom = -1.0*_frustum_size;
    _left   = -(double)w*_frustum_size/(double)h;
    _right  = -_left;

    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    glOrtho(_left,_right,_bottom,_top,_zNear*_frustum_size,_zFar*_frustum_size);
