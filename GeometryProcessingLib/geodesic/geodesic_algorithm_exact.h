...
namespace Another
{
	...
	private:
		...
		void construct_propagated_intervals(bool invert, 
			...
			Face_handle face,		//constructs iNew from the rest of the data
		...
		interval_pointer best_first_interval(Surface_point& point, 
		...
		list_pointer interval_list(Halfedge_handle e)
		{
		...
		void list_edges_visible_from_source(Mesh_element_base* p,
			std::vector<Halfedge_handle>& storage); //used in initialization

		long visible_from_source(Surface_point& point);	//used in backtracing

		void best_point_on_the_edge_set(Surface_point& point, 
			std::vector<Halfedge_handle> const& storage,
			interval_pointer& best_interval,
			double& best_total_distance,
			double& best_interval_position);

		void possible_traceback_edges(Surface_point& point, 
			std::vector<Halfedge_handle>& storage);
	...
	inline void GeodesicAlgorithmExact::best_point_on_the_edge_set(Surface_point& point, 
		std::vector<Halfedge_handle> const& storage,
		interval_pointer& best_interval,
		double& best_total_distance,
		double& best_interval_position)
	...
	inline void GeodesicAlgorithmExact::possible_traceback_edges(Surface_point& point, 
		std::vector<Halfedge_handle>& storage)
	{
		storage.clear();
		...
		{
			Vertex_handle v = static_cast<Vertex_handle>(point.base_element());
			for(unsigned i=0; i<v->adjacent_faces().size(); ++i)
			{
				Face_handle f = v->adjacent_faces()[i];
				storage.push_back(f->opposite_edge(v));
			}
		}
		else if(point.type() == EDGE)
		{
			Halfedge_handle e = static_cast<Halfedge_handle>(point.base_element());
			for(unsigned i=0; i<e->adjacent_faces().size(); ++i)
			{
				Face_handle f = e->adjacent_faces()[i];
		...
		{
			Face_handle f = static_cast<Face_handle>(point.base_element());
	...
	inline long GeodesicAlgorithmExact::visible_from_source(Surface_point& point)	//negative if not visible
	{
		...
		else if(point.type() == VERTEX)		
		{
			...
			{
				Halfedge_handle e = v->adjacent_edges()[i];
				list_pointer list = interval_list(e);
	...
	inline void GeodesicAlgorithmExact::list_edges_visible_from_source(Mesh_element_base* p,
		std::vector<Halfedge_handle>& storage)
	{
		...
		if(p->type() == FACE)
		{
			Face_handle f = static_cast<Face_handle>(p);
		...
		{
			Halfedge_handle e = static_cast<Halfedge_handle>(p);
			storage.push_back(e);
		}
		else			//VERTEX
		{
			Vertex_handle v = static_cast<Vertex_handle>(p);
			for(unsigned i=0; i<v->adjacent_edges().size(); ++i)
			{
	...
	inline void GeodesicAlgorithmExact::initialize_propagation_data()
	{
		...
		std::vector<Halfedge_handle> edges_visible_from_source;
		for(unsigned i=0; i<m_sources.size(); ++i)		//for all edges adjacent to the starting vertex			
		{
			...
			{
				Halfedge_handle e = edges_visible_from_source[j];
	...
	inline void GeodesicAlgorithmExact::propagate(std::vector<Surface_point>& sources,
		double max_propagation_distance,			//propagation algorithm stops after reaching the certain distance from the source
		std::vector<Surface_point>* stop_points)
	{
		...
		while(!m_queue.empty())
		{
			...
			Halfedge_handle edge = min_interval->edge();
			...
			for(unsigned i=0; i<edge->adjacent_faces().size(); ++i)		//two possible faces to propagate
			{
				...
				Face_handle face = edge->adjacent_faces()[i];			//if we come from 1, go to 2
				Halfedge_handle next_edge = face->next_edge(edge,edge->v0());
	...
	inline bool GeodesicAlgorithmExact::check_stop_conditions(unsigned& index)
	{
		...
		while(index < m_stop_vertices.size())
		{
			...
			Halfedge_handle edge = v->adjacent_edges()[0];				//take any edge
		...
		unsigned num_candidates)
	{
		...
		Halfedge_handle edge = list->edge();
	...
	inline unsigned GeodesicAlgorithmExact::best_source(Surface_point& point,			//quickly find what source this point belongs to and what is the distance to this source
		double& best_source_distance)
	{
		double best_interval_position; 
		unsigned best_source_index;
	...
	inline interval_pointer GeodesicAlgorithmExact::best_first_interval(Surface_point& point, 
		double& best_total_distance, 
		double& best_interval_position, 
		unsigned& best_source_index)
	{
		...
		else if(point.type() == FACE)		
		{
			...
			for(unsigned i=0; i<3; ++i)
			{
				Halfedge_handle e = f->adjacent_edges()[i];
				list_pointer list = interval_list(e);
	...
	inline void GeodesicAlgorithmExact::trace_back(Surface_point& destination,		//trace back piecewise-linear path
		std::vector<Surface_point>& path)
	{					
		path.clear();
		double best_total_distance; 
		...
		{
			std::vector<Halfedge_handle> possible_edges;
			...
			while(visible_from_source(path.back()) < 0)		//while this point is not in the direct visibility of some source (if we are inside the FACE, we obviously hit the source)
			{
				...
				Halfedge_handle e = interval->edge();
				double local_epsilon = SMALLEST_INTERVAL_RATIO*e->length();
				if(position < local_epsilon)
				{
					path.push_back(Surface_point(e->v0()));
				}
				else if(position > e->length()-local_epsilon)
				{
					path.push_back(Surface_point(e->v1()));
				}
				else
				{
					double normalized_position = position/e->length();
					path.push_back(Surface_point(e, normalized_position));
				}
			}
		}

		Surface_point& source = static_cast<Surface_point&>(m_sources[source_index]);
