...
public:
	...
	unsigned closest_vertices(Surface_point* p, 
								 std::vector<Vertex_handle>* storage = NULL);		//list vertices closest to the point
...
inline unsigned Mesh::closest_vertices(Surface_point* p, 
										  std::vector<Vertex_handle>* storage)
{
	...
	if(p->type() == VERTEX)
	{
		if(storage)
		{
			storage->push_back(static_cast<Vertex_handle>(p->base_element()));
		}
		return 1;
	}
	else if(p->type() == FACE)
	{
		if(storage)
		{
			Vertex_handle* vp= p->base_element()->adjacent_vertices().begin();
	...
	{
		Halfedge_handle edge = static_cast<Halfedge_handle>(p->base_element());
		...
		if(storage)
		{
			...
			{
				Face_handle face = edge->adjacent_faces()[i];
...
inline void Mesh::build_adjacencies()
{
	...
	for(unsigned i=0; i<m_faces.size(); ++i)
	{
		...
		for(unsigned j=0; j<3; ++j)
		{
			Vertex_handle v = f.adjacent_vertices()[j];
	...
	for(unsigned i=0; i<m_edges.size(); ++i)
	{
		...
		{
			Vertex_handle v = e.adjacent_vertices()[j];
	...
	for(unsigned i=0; i<m_edges.size(); ++i)
	{
		...
		{
			Face_handle f = e.adjacent_faces()[j];
			assert(count[f->id()]<3);
	...
	for(unsigned i=0; i<m_faces.size(); ++i)
	{
		...
		for(unsigned j=0; j<3; ++j)		//compute angle adjacent to the vertex j
		{
			...
			{
				Vertex_handle v = f.adjacent_vertices()[(j + k)%3];
...
inline bool Mesh::verify()		//verifies connectivity of the mesh and prints some debug info
{
	...
	std::vector<Face_handle> stack(1,&m_faces[0]);
	...
	while(!stack.empty())
	{
		...
		for(unsigned i=0; i<3; ++i)
		{
			...
			Face_handle f_adjacent = e->opposite_face(f);
...
inline void fill_surface_point_structure(geodesic::Surface_point* point, 
										 double* data, 
										 Mesh* mesh)
{
	point->set(data);
...
inline void fill_surface_point_double(geodesic::Surface_point* point, 
									  double* data, 
									  long mesh_id)
{
	data[0] = point->x();
