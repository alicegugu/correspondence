...
namespace Another
{
	...
	class SubdivisionNode: public Surface_point
	{
	...
	public: 
		...
		Surface_point& surface_point(){return static_cast<Surface_point&>(*this);};
	...
	public:
		...
		GeodesicAlgorithmGraphBase<Node>(mesh)
		{
			...
			{
				Vertex_handle v = &mesh->vertices()[i];
		...
		void set_subdivision_level(unsigned subdivision_level)
		{
			...
			{
				Halfedge_handle e = &m_mesh->edges()[i];
	...
	protected:
		...
		unsigned node_indexx(Halfedge_handle e)
		{
			return e->id()*m_subdivision_level + m_mesh->vertices().size();
		};

	private:
		void list_nodes(Mesh_element_base* p,		//list nodes that belong to this mesh element
			std::vector<node_pointer>& storage,
	...
	inline void GeodesicAlgorithmSubdivision::list_nodes(Mesh_element_base* p,
		std::vector<node_pointer>& storage,
	...
	void GeodesicAlgorithmSubdivision::list_nodes_visible_from_source(Mesh_element_base* p,
		std::vector<node_pointer>& storage)
	{
		assert(p->type() != UNDEFINED_POINT);
	...
	void GeodesicAlgorithmSubdivision::list_nodes_visible_from_node(node_pointer node, //list all nodes that belong to this mesh element
		...
		double threshold_distance)
	{
		Mesh_element_base* p = node->base_element();
		assert(p->type() != UNDEFINED_POINT);
		...
		if(p->type() == VERTEX)
		{
			...
			for(unsigned i=0; i<v->adjacent_edges().size(); ++i)
			{
				...
				Vertex_handle v_opposite = e->opposite_vertex(v);
			...
			for(unsigned i=0; i<v->adjacent_faces().size(); ++i)
			{
				...
				Halfedge_handle e = f->opposite_edge(v);
				list_nodes(e, storage, threshold_distance);
			}
		}
		else if(p->type() == EDGE)
		{
			...
			Vertex_handle v0 = e->adjacent_vertices()[0];
			Vertex_handle v1 = e->adjacent_vertices()[1];
