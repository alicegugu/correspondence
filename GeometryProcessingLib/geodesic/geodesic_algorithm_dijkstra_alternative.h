...
public:
	...
	virtual unsigned best_source(Surface_point& point,			//quickly find what source this point belongs to and what is the distance to this source
...
inline unsigned GeodesicAlgorithmDijkstraAlternative::best_source(Surface_point& point,			//quickly find what source this point belongs to and what is the distance to this source
																	   double& best_source_distance)
{
	if(point.type() == VERTEX)		
	{
		Vertex_handle v = (Vertex_handle)point.base_element();
	...
	{
		std::vector<Vertex_handle> possible_vertices;		//initialize vertices directly visible from sources
		m_mesh->closest_vertices(&point, &possible_vertices);

		best_source_distance = GEODESIC_INF;
		Vertex_handle min_vertex = NULL;
		for(unsigned i=0; i<possible_vertices.size(); ++i)
		{
			Vertex_handle v = possible_vertices[i];
...
inline void GeodesicAlgorithmDijkstraAlternative::trace_back(Surface_point& destination,		//trace back piecewise-linear path
														   std::vector<Surface_point>& path) 
{
	path.clear();
	...
	if(destination.type() != VERTEX)		//snap to the closest vertex
	{
		...
		assert(min_vertex);
		path.push_back(Surface_point(min_vertex));
	}
	...
	{
		node = node->previous();
		path.push_back(Surface_point(node->vertex()));
	}
...
inline void GeodesicAlgorithmDijkstraAlternative::propagate(std::vector<Surface_point>& sources,
															...
															std::vector<Surface_point>* stop_points)   //or after ensuring that all the stop_points are covered
{
	...
	std::vector<Vertex_handle> visible_vertices;		//initialize vertices directly visible from sources
	for(unsigned i=0; i<m_sources.size(); ++i)
	{
		Surface_point* s = &m_sources[i];
		m_mesh->closest_vertices(s, &visible_vertices);
		for(unsigned j=0; j<visible_vertices.size(); ++j)
		{
			Vertex_handle v = visible_vertices[j];
